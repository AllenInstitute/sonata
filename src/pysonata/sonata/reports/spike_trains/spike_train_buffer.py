...
class STMPIBuffer(STCSVBuffer):
    def __init__(self, cache_dir=None, default_population=None, **kwargs):
        self.mpi_rank = kwargs.get('MPI_rank', sonata_world_comm.MPI_rank)
        self.mpi_size = kwargs.get('MPI_size', sonata_world_comm.MPI_size)
    ...
    def _cache_fname(self, cache_dir):
        ...
        return os.path.join(self._cache_dir, '.sonata.spikes.cache.node{}.csv'.format(self.mpi_rank))

    def _all_cached_files(self):
        return [os.path.join(self._cache_dir, '.sonata.spikes.cache.node{}.csv'.format(r)) for r in range(sonata_world_comm.MPI_size)]
